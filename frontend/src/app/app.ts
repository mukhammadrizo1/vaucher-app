...
Адрес: 100047, город Ташкент, Мирабадский район, улица Бухара, дом 123.`,
  ...
  async confirmSendDetails() {
    ...
    try {
      ...
      const existingDocs = await this.documentApiService.getDocumentDetails();
      ...
      if (existingDocs.length > 0) {
        const ids = existingDocs
          ...
          .filter((id?: string): id is string => !!id && /^\d+$/.test(id))
      ...
      // Save to Neon Database via NestJS API
      await this.documentApiService.saveDocumentDetails(documentDetails);
